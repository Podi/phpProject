...
class Reg_Magic_Block {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'reg-magic-blocks-forms', 'rm_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'premium_active'=>true ) );
        wp_register_script(
            'rm_ctabs_script',
        ...
        wp_enqueue_style('rm_magic_front_style', RM_BASE_URL . 'public/css/style_rm_front_end.css', array(), $this->version, 'all');
       
        if(defined('REGMAGIC_ADDON')) {
            wp_enqueue_style('rm_magic_front_style_addon', RM_ADDON_BASE_URL . 'public/css/style_rm_front_end.css', array(), $this->version, 'all');
        ...
        if($theme == 'default') {
            ...
            wp_enqueue_script( 'rm-block-editor', plugins_url( 'rm-editor.js', __FILE__ ), array( 'jquery'), false, true );
            
            $premium_active = false;
            if(defined('REGMAGIC_ADDON')){
                $premium_active = true;
            }
            wp_localize_script( 'reg-magic-blocks-forms', 'rm_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'premium_active'=> $premium_active ) );
	...
	public function reg_magic_block_register() {
        ...
        wp_enqueue_style( 'rm_material_icons', RM_BASE_URL . 'admin/css/material-icons.css' );
			wp_enqueue_style( 'rm_blocks_custom_tabs', RM_BASE_URL . 'public/css/rm_custom_tabs.css' );
			if(defined('REGMAGIC_ADDON')){
                            wp_enqueue_style( 'rm_addon_front_end', RM_ADDON_BASE_URL . 'public/css/style_rm_front_end.css' );
                        }
                            wp_enqueue_script( 'rm-block-editor', plugins_url( 'rm-editor.js', __FILE__ ), array( 'jquery'), false, true );
        
                        wp_register_style( 'reg-magic-gutenberg', RM_BASE_URL . 'blocks/reg-magic-gutenberg-style.css',array(), $this->version, 'all' );
                        wp_register_script(
                            'rm_ctabs_script',
                            RM_BASE_URL . 'public/js/rm_custom_tabs.js',
                            array(),
					...
					'render_callback' => array( $this, 'regmagic_login' ),
                        ...
                        if(defined('REGMAGIC_ADDON')){
                            register_block_type(
                                    ...
                                    array(
                                            'editor_script'   => 'reg-magic-blocks-forms',
                                            'editor_style'    => 'rm_addon_front_end',
	...
	public function regmagic_forms( $atts ) {
            ...
            if(!empty($fid)){
                ...
                echo '<div class="rm-gutenberg-empty-content">'._('Please Select Form','custom-registration-form-builder-with-submission-manager').'</div>';
            }
