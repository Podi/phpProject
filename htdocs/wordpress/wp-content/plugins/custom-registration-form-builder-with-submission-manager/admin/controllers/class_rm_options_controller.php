    ...
    public function admin_menu($model, RM_Setting_Service $service, $request, $params){
        if ($this->mv_handler->validateForm("options_admin_menu")){
            if ($request->req['restore'] == 'false'){
                $options = array();
                $options['admin_order'] = "";
                if(isset($request->req['enable_admin_order'])) {
                    $options['enable_admin_order'] = 'yes';
                } else {
                    $options['enable_admin_order'] = 'no';
                }
                $hidden_menu['num_hidden_menus'] = 0;
                ...
                foreach ($menus as $slug) {
                    ...
                    array_push($menu_order,$new_order);
                }
                $options['admin_order'] = $menu_order;
                
                $service->set_model($model);
                $service->save_options($options);
                $service->save_options($hidden_menu);
                if (class_exists("Registration_Magic_Addon")) {
                    $service->save_options($inbox_badge);
                }
                $data = $service->get_options();
                ...
                $service->reset_option('inbox_badge');
                $data = $service->get_options();
                $view = $this->mv_handler->setView('options_admin_menu');
                $view->render($data);
                $refresh_time = 0;
