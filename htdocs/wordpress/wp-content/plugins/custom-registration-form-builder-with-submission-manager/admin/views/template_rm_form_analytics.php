...
<pre class='rm-pre-wrapper-for-script-tags'><script>
    ...
    <div class="rmagic-analytics rmagic-analytics-wrap">
...
<?php 
    include RM_ADMIN_DIR.'views/template_rm_form_analytics-dummy.php';
    ?>
...
<div class="rm-left-stats-box">
    <div class="rm-analytics-stat-counter">
        <div class="rm-analytics-stat-counter-value"><?php echo esc_html($data->analysis->failure_rate); ?><span class="rm-counter-value-dark">%</span></div>
...
<div class="rm-right-stats-box">
    <div class="rm-analytics-stat-counter">
        <div class="rm-analytics-stat-counter-value"><?php echo esc_html($data->analysis->avg_filling_time); ?><span class="rm-counter-value-dark">s</span></div>
...
<div class="rm-analytics-table-wrapper rm-analytics-table-dummy">    
 <table class="rm-form-analytics rmagic-table">
    <th>#</th>
    ...
    foreach ($data->stat_data as $stat) {
        ...
        <tr>
            ...
            <td><?php echo esc_html($visited_on); ?></td>
            <td><?php echo esc_html($submitted_on); ?></td>
            <td><?php
                echo esc_html($stat->time_taken);
                if ($stat->time_taken)
                    echo "s";
                ?></td>
        </tr>
...
if ($data->total_pages > 1):
    ...
    <ul class="rmpagination">
        ...
        if ($data->curr_page > 1):
            ?>
            <li><a href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_form_id=<?php echo esc_attr($data->current_form_id); ?>&rm_reqpage=1"><?php echo RM_UI_Strings::get('LABEL_FIRST'); ?></a></li>
            <li><a href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_form_id=<?php echo esc_attr($data->current_form_id); ?>&rm_reqpage=<?php echo esc_attr($data->curr_page - 1); ?>"><?php echo RM_UI_Strings::get('LABEL_PREVIOUS'); ?></a></li>
            <?php
        endif;
        ...
        for ($i = $first_visible_page; $i <= $last_visible_page; $i++):
            if ($i != $data->curr_page):
                ?>
                <li><a href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_form_id=<?php echo esc_attr($data->current_form_id); ?>&rm_reqpage=<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></a></li>
            <?php else:
                ?>
                <li><a class="active" href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_form_id=<?php echo esc_attr($data->current_form_id); ?>&rm_reqpage=<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></a></li>
            <?php
            endif;
        endfor;
        if ($data->total_pages > $max_pages_without_abb):
        ...
        if ($data->curr_page < $data->total_pages):
            ?>
            <li><a href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_form_id=<?php echo esc_attr($data->current_form_id); ?>&rm_reqpage=<?php echo esc_attr($data->curr_page + 1); ?>"><?php echo RM_UI_Strings::get('LABEL_NEXT'); ?></a></li>
            <li><a href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_form_id=<?php echo esc_attr($data->current_form_id); ?>&rm_reqpage=<?php echo esc_attr($data->total_pages); ?>"><?php echo RM_UI_Strings::get('LABEL_LAST'); ?></a></li>
            <?php
        endif;
        ?>
    </ul>
