                    ...
                    if (!empty($data->login_logs) && (is_array($data->login_logs) || is_object($data->login_logs))){
                        ...
                        foreach ($data->login_logs as $login_log){
                            ...
                            <tr class="rm-login-result <?php echo ($login_log->status==1)?'rm-login-success':'rm-login-failed'; ?>">
                                <td><div class="rm-login-user-time-log"><?php echo esc_html(RM_Utilities::localize_time($login_log->time)); ?></div></td>
                                <td>
                                ...
                                <td><div class="rm-login-user-ip <?php echo ($login_log->ban==1)?'rm-login-boolean-result rm-login-false':''; ?>" <?php echo ($login_log->ban==1)?'style="color:#c21106;"':''; ?>><?php echo $login_log->ip == '::1' ? 'localhost' : esc_html($login_log->ip) ?><?php echo ($login_log->ban==1)?'<i class="fa fa-times"></i>':''; ?></div></td>
                            ...
                            <div id="rm-login-user-details-<?php echo esc_attr($login_log->id); ?>" class="rm-login-user-details" style="display:none">
                                    ...
                                    <div class="rm-modal-container">
                                        <div class="rm-login-user-details-dialog">
                                            <div class="rm-login-details-lf">
                                                <div class="rm-login-user-profile">
                                                    ...
                                                    <div class="rm-register-details">
                                                        ...
                                                        <span class="rm-dbfl rm-registered-date"> <?php echo esc_html(date('j M Y', strtotime($user->user_registered))); ?> </span>
                                                    </div>
                                                    <div class="rm-view-register-user"><a href="?page=rm_user_view&user_id=<?php echo esc_attr($user->ID); ?>"><?php _e('View', 'custom-registration-form-builder-with-submission-manager'); ?></a></div>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                            <div class="rm-login-details-rf">
                                                ...
                                                ?>
                                                <div class="rm-login-user-info"><?php _e('Login Result', 'custom-registration-form-builder-with-submission-manager'); ?><span><?php echo esc_html($login_result); ?></span></div>
                            ...
                            <div id="rm-send-email-<?php echo esc_attr($login_log->id); ?>" class="rm-login-user-details rm-send-email" style="display:none">
                                <div class="rm-modal-overlay"></div>
                                <div class="rm-modal-wrap">
        ...
        if ($data->total_pages > 1):
            ...
            <ul class="rmpagination">
                ...
                for ($i = $first_visible_page; $i <= $last_visible_page; $i++):
                    if ($i != $data->curr_page):
                        ?>
                        <li><a href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_reqpage=<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></a></li>
                    <?php else:
                        ?>
                        <li><a class="active" href="?page=<?php echo esc_attr($data->rm_slug) ?>&rm_reqpage=<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></a></li>
                    <?php
                    endif;
                endfor;
                if ($data->total_pages > $max_pages_without_abb):
