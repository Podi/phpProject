 jQuery(document).ready(function(){
    ...
    jQuery(".rmagic .mce-tinymce iframe ").contents().find("body").attr("style","background-color:#fff;font-size: 16px;");
     ...
     if(jQuery('#id_show_selected_icon').length) {
         var rm_field_icon = jQuery('#id_show_selected_icon').html();
         jQuery('#id_show_selected_icon').html(rm_field_icon.replace("&amp;", "&"));
     }
     
       rmDashboardUserImages();
...
(function (RM_jQ) {
    ...
    RM_jQ(function () {
        ...
        RM_jQ('.rm_sortable_form_rows').sortable({
            axis: 'y',
            opacity: 0.7,
            handle: '.rm_sortable_handle',
            revert: true,
            ...
            update: function (event, ui) {
                ...
                var data = {
                    action: 'rm_sort_form_rows',
                    'rm_slug': 'rm_field_set_row_order',
                    'rm_sec_nonce': rm_admin_vars.nonce,
                    data: list_sortable
                };
...
//        RM_jQ("#scroll").hide();
        ...
        switch (field_type) {
            case 'Textbox' :
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0").not("#rm_field_default_value");
                break;
            case 'WCBilling' :
            case 'WCShipping' :
            case 'Fname' :
            ...
            case 'Privacy':
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0, #rm_field_max_length, #rm_field_min_length, #rm_field_is_required-0, #rm_field_show_on-0, #rm_field_is_editable-0, #rm_sub_heading");
                RM_jQ('#rm_field_placeholder').parents(".rmrow").hide();
            ...
            case 'WCBillingPhone' :
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0, #rm_field_show_on-0, #rm_field_is_editable-0, #rm_field_max_length, #rm_field_min_length");
                break;
                
            case 'Hidden':
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0, #rm_field_placeholder, #rm_field_helptext, #rm_field_class, #rm_field_max_length, #rm_field_min_length, #rm_field_is_required-0, #rm_field_show_on-0, #rm_field_is_editable-0").not("#rm_field_default_value");
        ...
        if (field_type === 'PGAvatar') {
            RM_jQ('input#field_user_profile-2').prop('checked',true);
            RM_jQ('input#define_new_user_meta').val('pm_user_avatar');
            RM_jQ('div#rm_user_meta_options').hide();
        }
        ...
        if (field_type === 'Number') {
            ...
            RM_jQ("#rm_field_min_length").parents(".rmrow").show();
            RM_jQ("#rm_field_min_length").prop('disabled', false);
        }
        ...
        if (field_type === 'Select') {
            RM_jQ("#rm_field_enable_search-0").parents(".rmrow").show();
            RM_jQ("#rm_field_enable_search-0").prop("disabled", false);
        }
    };
    ...
    RM_jQ.rm_do_action = function (form_id, slug) {
        ...
        } else if(slug == 'rm_submission_remove'){
            var choice = confirm("This action cannot be reversed. Are you sure you want to delete the selected submissions?");
            if(choice) {
                var form = RM_jQ("form#" + form_id);
                form.children('input#rm_slug_input_field').val(slug);
                form.submit();
...
function rmDashboardUserImages() {
    jQuery(".rm-dash-avatar").each(function (index) {
        var img = jQuery(this);
        setTimeout(function(){
...
jQuery(document).ready(function(){
    ...
    if(themeSelector === 'default'){
        jQuery('#layout_left_container').addClass('rm-disabled-layout');
        jQuery('#layout_two_columns_container').addClass('rm-disabled-layout');
        jQuery('input#layout_radio-2').prop('checked', true);
    }
    
    jQuery('#theme_dropdown').change(function () {
        if(jQuery(this).val() == 'default'){
            jQuery('#layout_left_container').addClass('rm-disabled-layout');
            jQuery('#layout_two_columns_container').addClass('rm-disabled-layout');
            jQuery('input#layout_radio-2').prop('checked', true);
        } else {
            jQuery('#layout_left_container').removeClass('rm-disabled-layout');
            jQuery('#layout_two_columns_container').removeClass('rm-disabled-layout');
        }
    });
});
jQuery(document).on('click','.rm-conditions-field-container li label', function(){
   jQuery(this).closest('li').find('input:radio').trigger('click');
});
jQuery(document).on('click','.rm-match-condition label', function(){
   jQuery(this).closest('.rm-match-condition').find('input:radio').trigger('click');
});



//Inbox Quick view Page


jQuery(document).ready(function ($) {
    // Get the total number of pages
    const pageCount = $('.rm-form-page').length;
...
//Inbox Quick view Modal 
    ...
    $(".rm-quick-view-button").each(function() {
       
        
        $(this).click(function() {
             $('.rm-page-wrapper .rm-form-page:first-child:first-child').css("display", "block");
             var submissionId = $(this).data("submission-id");
            //console.log(submissionId);
            //populateModal(submissionData);
