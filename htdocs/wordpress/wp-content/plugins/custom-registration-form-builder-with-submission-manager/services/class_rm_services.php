...
class RM_Services {
    ...
    public function add_default_cstatuses($form_id) {
        $form = new RM_Forms();
        ...
        $custom_status= array();
        if(!isset($form_options->custom_status) || empty($form_options->custom_status)) {
            $form_options->custom_status = array();
        }

        foreach($statuses as $status => $color) {
            $custom_status= array();
            $custom_status['label']= $status;
            $custom_status['desc']= '';
            $custom_status['color']= $color;
            $custom_status['cs_show_frontend']= 0;
            ...
            $form->set_form_options($form_options);
            $form->update_into_db();
        }
    }
    ...
    public function add_default_crons($form_id) {
        ...
        $task_params[] = array(
            ...
            'rm-task-slide' => 'on',
            'rmc_task_name' => 'Activate User on Payment Completion',
            'rmc_task_description' => '',
            ...
            'rmc_enable_pay_status_rule' => '1',
            'rmc_rule_pay_status' => 
                array(
                    0 => 'completed',
                ),
            'rmc_action_user_acc' => 'activate',
            'rmc_action_send_mail_sub' => '',
            'rmc_action_send_mail_body' => '',
            'rmc_task_type' => 'manual',
            'rmc_task_first_execution' => '',
            'rmc_task_schedule' => 'hourly',
        );
        ...
        $task_params[] = array(
            ...
            'rmc-task-edit-form-subbed' => 'yes',
            'rm-task-slide' => 'on',
            'rmc_task_name' => 'Activate User After 2 Days of Registration',
            'rmc_task_description' => '',
            'rmc_enable_sub_time_rule' => '1',
            'rmc_enable_sub_time_rule_older_than' => '1',
            'rmc_rule_sub_time_older_than_age' => '2',
            'rmc_rule_sub_time_younger_than_age' => '',
            'rmc_rule_fv_fids' => 
                array(
                    0 => '',
        ...
        foreach($task_params as $params) {
            $task_result = $chronos_service->process_request($params);
            $wpdb->query("UPDATE {$table_name} SET `is_active` = 0 WHERE `task_id` = {$task_result->props['task_id']}");
        }

    }

    public function add_sample_subs() {
        ...
        for ($x = 1; $x <= 4; $x++) {
            ...
            switch($x) {
                case 1:
                    ...
                    $field_data->value = 'sampleuser@metagauss.com';
                    $field_data->type = 'Email';
                    $db_data[$x] = $field_data;
                    break;
                case 2:
                    $field_data->label = 'Name';
                    $field_data->value = 'Sample User';
                    $field_data->type = 'Textbox';
                    $db_data[$x] = $field_data;
                    break;
                case 3:
                    $field_data->label = 'Message';
                    $field_data->value = 'This is just a sample form submission';
                    $field_data->type = 'Textarea';
                    $db_data[$x] = $field_data;
                    break;
                case 4:
                    $field_data->label = 'Phone';
                    $field_data->value = '(000)-000-0000';
                    $field_data->type = 'Textbox';
                    $db_data[$x] = $field_data;
                    break;
            }
        }
        $sub_detail = $service->save_submission(1, $db_data, 'sampleuser@metagauss.com');
        if(isset($sub_detail)) {
            $stat_entry = $service->create_stat_entry(array('form_id' => 1));
            $service->update_stat_entry($stat_entry, 'update', $sub_detail->submission_id);
            $this->update_custom_statuses(2, $sub_detail->submission_id, 1, 'clear_all');
        }
    }
    ...
    public function add_user_form() {
        ...
        $this->add_default_cstatuses($form_id);
        $this->add_default_crons($form_id);

        return $form_id;
    }
    
    public function create_basic_form($template){
        $form_id = $this->model->insert_into_db();
        $form = new RM_Forms();
        $form->load_from_db($form_id);
        $form_type = $form->get_form_type();

        $this->add_template_form_fields($form_id, $form_type, $template);

        return $form_id;
    }

    public function add_template_form_fields($form_id, $form_type, $template){
        $templateService = new RM_form_template_service;
        
        if($form_type){
            $templateService->create_registration_template($form_id, $form_type, $template);
        }
        else{
            $templateService->create_contact_template($form_id, $form_type, $template);
        }
    ...
    public static function import_form_first_ajax() {
        if((current_user_can('manage_options') || current_user_can('rm_form_managemanage_options') )&& check_ajax_referer( 'rm_import_first', 'rm_ajaxnonce' ))
        {
            $form_id = null;
    ...
    public function import_form($data_value) {
        ...
        foreach ($data_value as $data_identifier => $data_value) {
            ...
            if ($data_identifier == 'FIELDS') {
                ...
                //echo "Field ID:-",$result,"</br>"; 
            } elseif ($data_identifier == 'ROWS') {
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $rm_sr->import_rows($form_id_new, $field_data);
                        // echo "Field ID:-",$result,"</br>"; 
                    }
                } else {
                    $result = $rm_sr->import_rows($form_id_new, $data_value);
                }
    ...
    public function import_rows($form_id_new, $rows) {
        $data = array();
        $data_format = array();
        $data_identifier = 'ROWS';

        $data_format = $this->get_format_specifer($data_identifier);
        foreach ($rows as $row_attr_name => $attr_value) {
            if ($row_attr_name != 'row_id') {
                if (empty($attr_value)) {
                    $data[$row_attr_name] = '';
                    ...
                    foreach($old_field_ids as $old_field_id) {
                        if(!empty($old_field_id)) {
                            array_push($new_field_ids, $this->field_id_array[$old_field_id]);
                        } else {
                            array_push($new_field_ids, '');
                        }
                    }
                    $data[$row_attr_name] = maybe_serialize($new_field_ids);
                } else
                    $data[$row_attr_name] = (string) $attr_value;
            } else
                $row_id_old = (int) $attr_value;

            if ($row_attr_name == 'form_id') {
                $data[$row_attr_name] = $form_id_new;
            }
        }

        $row_id_new = RM_DBManager::insert_row($data_identifier, $data, $data_format);
        $this->row_id_array[$row_id_old] = $row_id_new;
        return $row_id_new;
    }
    ...
    public function get_all_form_fields_by_rows($form_id) {
        if ((int) $form_id)
            return RM_DBManager::get_rows_fields_by_form_id($form_id);
        else
            return false;
    }
    
    public function get_all_form_rows($form_id, $page_no = null) {
        if ((int) $form_id)
            return RM_DBManager::get_rows_by_form_id($form_id, $page_no);
        else
            return false;
    }

    public function get_all_fields_by_row($row) {
        $fields = array();
        if (!empty($row->field_ids)) {
            $row->field_ids = maybe_unserialize($row->field_ids);
            foreach($row->field_ids as $field_id) {
                if(!empty($field_id)) {
                    array_push($fields,RM_DBManager::get('FIELDS',array('field_id'=>intval($field_id)),array('%d'),'row'));
                } else {
                    array_push($fields,'');
                }
            }
   ...
   public function create_default_username_field($form_id, $has_rows = true) {
    ...
    public function create_default_field_row($form_id, $field_id) {
        $row = new RM_Rows;
    ...
    public function remove_form_rows($form_id) {
        if (is_array($form_id))
            foreach ($form_id as $formId)
                RM_DBManager::delete_form_rows($formId);
        elseif ((int) $form_id)
            RM_DBManager::delete_form_rows($form_id);
        else
            throw new InvalidArgumentException("Invalid Form ID '$form_id'.");
    }
    ...
    public function duplicate_form_rows($form_id, $ids) {
        if (is_array($form_id)) {
            foreach ($form_id as $formId) {
    ...
    public function duplicate_field($field_id, $form_id, $skip_primaries = false) {
        $model = new RM_Fields;
    ...
    public function duplicate_row($row_id, $form_id, $field_ids = array(), $page_no = 1) {
        $model = new RM_Rows;
        $model->load_from_db($row_id, false);
        $model->set_form_id($form_id);
        if(is_array($field_ids) && empty($field_ids)) {
            $temp_ids = array();
            foreach($model->field_ids as $id) {
                if(empty($id))
                    array_push($temp_ids,'');
                else
                    array_push($temp_ids, $this->duplicate_field($id, $form_id, true));
            }
            $model->set_field_ids($temp_ids);
            //$model->set_row_order(RM_DBManager::get_row_count_by_form_id($form_id, $page_no));
        } else {
            $model->set_field_ids($field_ids);
        }
        $model->insert_into_db();
        $this->refresh_row_order($form_id, $page_no);
    }
    
    public function refresh_row_order($form_id, $page_no) {
        $rows = $this->get_all_form_rows($form_id, $page_no);
        if(!empty($rows) && is_array($rows)) {
            foreach($rows as $index => $row) {
                $temp_rows[$index] = intval($row->row_id);
            }
            RM_DBManager::set_row_order($temp_rows);
        }
    }
    ...
    public function set_row_column_order($list, $row_id, $page_id){
        RM_DBManager::set_row_column_order($list, $row_id, $page_id);
    }
    public function set_row_order($list) {
        RM_DBManager::set_row_order($list);
    }
    ...
    public function get_submissions_to_export(RM_Submission_Filter $filter=null, $sub_ids = array(), $form_id = null) {
        ...
        $form_id = !empty($filter) ? $filter->get_form() : $form_id;
        
        if (!(int) $form_id)
            return false;

        $fields = $this->get_all_form_fields_by_rows($form_id);
    ...
    public function create_csv($data, $file_name="rm_submissions") {

        $csv_name = $file_name . time() . mt_rand(10, 1000000);
        ...
        foreach ($data as $a) {
            $a = array_map("RM_Utilities::prepare_csv_content", $a);
            if (!fputcsv($csv, $a))
                return false;
        }
    ...
    public function manage_form_page($action, $form_id, $page_no = null, $new_page_name = null) {
        if(defined('REGMAGIC_ADDON')) {
            ...
            return $addon_service->manage_form_page($action, $form_id, $this, $page_no, $new_page_name);
        }
        $form = new RM_Forms;
        $form->load_from_db($form_id);
    ...
    public function manage_form_row($action, $request) {

        switch ($action) {
            case 'add_row':
                $new_row = new RM_Rows;
                $new_row->form_id = absint($request->req['form-id']);
                $new_row->page_no = absint($request->req['page-no']);
                $new_row->columns = sanitize_text_field($request->req['columns']);
                $new_row->class = strtolower(sanitize_text_field($request->req['class']));
                $new_row->gutter = sanitize_text_field($request->req['gutter']) == '' ? 10 : absint($request->req['gutter']);
                ...
                return $new_row->insert_into_db();
                break;

            case 'update_row':
                $new_row = new RM_Rows;
                $new_row->load_from_db(absint($request->req['rm_row_id']));
                $new_row->page_no = absint($request->req['page-no']);
                $new_row->columns = sanitize_text_field($request->req['columns']);
                $new_row->class = strtolower(sanitize_text_field($request->req['class']));
                $new_row->gutter = absint($request->req['gutter']);
                $new_row->bmargin = absint($request->req['bmargin']);
                $new_row->width = absint($request->req['width']);
                $new_row->heading = sanitize_text_field($request->req['heading']);
                $new_row->subheading = sanitize_text_field($request->req['subheading']);
                ...
                } elseif(count($new_row->field_ids) > $new_row->valid_columns[$new_row->columns]) {
                    $reverse_ids = array_reverse($new_row->field_ids);
                    foreach($reverse_ids as $index => $field) {
                        if(count($reverse_ids) > $new_row->valid_columns[$new_row->columns]) {
                            if(empty($field)) {
                                unset($reverse_ids[$index]);
                            }
                        }
                    }
                    $new_row->field_ids = array_values(array_reverse($reverse_ids));
                }
                if(empty($new_row->field_ids)) {
                    array_push($new_row->field_ids,'');
                }
                return $new_row->update_into_db();
                break;

            case 'delete_row':
                return $this->remove(absint($request->req['rm_row_id']), 'ROWS');
                break;
        }
 ...
 public function get_review_event()
    ...
    public function get_editable_rows($form_id, $is_admin = false) {
        if(defined('REGMAGIC_ADDON')) {
            $addon_service = new RM_Services_Addon();
            return $addon_service->get_editable_rows($form_id, $this, $is_admin);
        }
    }
    ...
    */
    ...
    public function update_submit_field_config($form_id, $config){
        ...
        if($form->load_from_db($form_id)) {            
            $form->form_options->form_btn_align = sanitize_text_field($config['btn_align']);            
            $sub_btn = trim(sanitize_text_field($config['submit_btn_label']));
    ...
    public function fcm_update_form($form_id, $config){ 
        ...
        if($form->load_from_db($form_id)) {
            ...
            if(isset($config['form_name']))
                $form_name = trim(sanitize_text_field($config['form_name']));
     ...
     */
    public function add_primary_fields($form_id, $has_rows = true){
        ...
        if(empty($form_options->hide_username)){
            ...
            if(empty($username_field)){
                $this->create_default_username_field($form_id, $has_rows);
            }
        }
    ...
    public function forms_template_lists($type=''){
        $template = array();
            $template[] = array('title'=>'Blank Form','description'=>'Start with a clean slate and develop your form, one field at a time.','id'=>'c0','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Blank Registration Form', 'description'=>'Just the essentials to start building user registration form.', 'id'=>'r0', 'type'=>'registration', 'ver'=>'standard','multipage'=> 0,'create_user'=>1 );
            $template[] = array('title'=>'Quick Contact Form','description'=>'Collects name, email and a multi-line message from visitors. All the basics of a contact form are covered here.','id'=>'c1','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Contact Form With Terms And Conditions','description'=>'A neat looking contact form with just the essentials, plus provision for accepting terms and conditions before sending.','id'=>'c2','type'=>'contact','ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Quick Suggestion Form','description'=>'Collect feedback from your users about the services you offer.','id'=>'c3','type'=>'contact','ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Generic Service Quote Form','description'=>'Generate leads by soliciting quotation requests from your website visitors for services or products you offer. Features cascading dropdown fields.','id'=>'c4','type'=>'contact','ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Business Inquiry Form','description'=>'Let other business contact you for B2B opportunities. Formatted to collect information on behalf of organizations.','id'=>'c5','type'=>'contact','ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Conference Registration Form','description'=>'Register visitors for conference which you plan to host.','id'=>'c6','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Automobile Information Form','description'=>'Gather information for car insurance, repairs, or services.','id'=>'c7','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Feature Request Form','description'=>'Make it easy for users to request new features for your products.','id'=>'c8','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Grade Book Form','description'=>'Collect information about student submitted assignments.','id'=>'c9','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'PayPal or Credit Card Payment Form','description'=>'Accept quick payments via PayPal Standard using this form.','id'=>'c10','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'IT Asset Form','description'=>'Allow your employees to connect with the IT department and submit their issues.','id'=>'c11','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Marriage Registration Form','description'=>'Find out who will be attending the wedding you have organized by online RSVP form.','id'=>'c12','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Employee Referral Form','description'=>'Allow your employees to refer eligible candidates and submit related information.','id'=>'c13','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Complaint Form','description'=>'Let your users register complaints with your customer service department.','id'=>'c14','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0); 
            $template[] = array('title'=>'Summer Camp Registration Form','description'=>'Use a single form to collect information about children and their guardians attending your summer camp.','id'=>'c15','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);           
            $template[] = array('title'=>'Boot Camp Registration Form','description'=>'An advanced template that adds almost all required fields for publishing a boot camp registration form. Terms and Conditions field is also included.','id'=>'c16','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Market Research Survey Form','description'=>'A ready to use survey form with multiple fields to better understand your audience demographic.','id'=>'c17','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Dance Workshop Registration Form','description'=>'A powerful template to get you started with enrolling participants for any type of dance workshop you are conducting.','id'=>'c18','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            $template[] = array('title'=>'Invoice Request Form','description'=>'Let your customers request invoices for their purchases through an intuitive request form with all the necessary fields to boot.','id'=>'c19','type'=>'contact', 'ver'=>'standard','multipage'=> 0,'create_user'=>0);
            
            //REG
            $template[] = array('title'=>'Quick Registration Form', 'description'=>'Collects minimum information during registration. Useful when you want it quick and fast!', 'id'=>'r1', 'type'=>'registration', 'ver'=>'standard','multipage'=> 0,'create_user'=>1 );
            $template[] = array('title'=>'Standard Registration Form', 'description'=>'Collects name, email and asks users to pick a username during registration.', 'id'=>'r2', 'type'=>'registration', 'ver'=>'standard','multipage'=> 0,'create_user'=>1 );
            $template[] = array('title'=>'Extended Registration Form', 'description'=>'Collects name, email, city, country, gender and phone number from users during registration.', 'id'=>'r3', 'type'=>'registration', 'ver'=>'standard','multipage'=> 0,'create_user'=>1 );
            if(defined('REGMAGIC_ADDON')){
            //Premium
                $template[] = array('title'=>'Product Purchase Form','description'=>'Let your customers select single or multiple products from a list and make payment using your preferred payment gateway.','id'=>'cp1','type'=>'contact','ver'=>'premium', 'multipage'=> 1, 'create_user'=>0);
                $template[] = array('title'=>'Job Application Form','description'=>'Solicit applications for openings in your organization by collecting MS Word or PDF resume files, along with skills profile.','id'=>'cp2','type'=>'contact','ver'=>'standard','multipage'=> 0, 'create_user'=>1 );
                $template[] = array('title'=>'Transfer of Pet Ownership Form','description'=>'Facilitate transfer of pet ownership from current owner to another using this form.','id'=>'cp3','type'=>'contact','ver'=>'premium','multipage'=> 0,'create_user'=>0);
                $template[] = array('title'=>'School Registration Form ','description'=>'Streamline school admission process by collecting children and guardian information.','id'=>'cp4','type'=>'contact','ver'=>'premium','multipage'=> 1,'create_user'=>0);
                $template[] = array('title'=>'Multi-Step Registration Form', 'description'=>'Spread over multiple pages, this form collects lots of useful information like name, email, date of birth, phone, complete address etc.', 'id'=>'rp1', 'type'=>'registration', 'ver'=>'premium','multipage'=> 1,'create_user'=>1 );
            }
        return $template;
    }
}