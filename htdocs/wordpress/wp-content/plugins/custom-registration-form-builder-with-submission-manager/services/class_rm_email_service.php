...
class RM_Email_Service
{
    ...
    public static function auto_responder($params,$token='')
    {
        ...
        $email_content .= wpautop(html_entity_decode($params->email_content)) . '<br><br>';
            ...
            foreach ($params->sub_data as $field_id => $val) {
                ...
                if (is_array($val->value)) {
                    ...
                    if (isset($val->value['rm_field_type']) && $val->value['rm_field_type'] == 'File') {
                        ...
                        $sub_data .= '<span class="key-val">' . $values . '</span><br/>';
                    }elseif (isset($val->value['rm_field_type']) && $val->value['rm_field_type'] == 'Address'){
                        unset($val->value['rm_field_type']);
                        foreach($val->value as $in =>  $value){
                           if(empty($value))
                    ...
                    elseif ($val->type == 'Radio' || $val->type == 'Select') {   
                       $sub_data .= '<span class="key-val">' . RM_Utilities::get_lable_for_option($field_id, $val->value). '</span><br/>';
                    }
                    else
                        $sub_data .= '<span class="key-val">' . nl2br($val->value) . '</span><br/>';
                }

                $sub_data .= "</div>";
            }
            $email_content = str_replace('{{SUBMISSION_DATA}}', $sub_data, $email_content);
        }
    ...
    {   
        ...
        } elseif($type=='form_user_payment_invoice')
        {
            $email_content= '<p>Hello {{FIRST_NAME}},</p>
            <p>Here’s the invoice for payment you made with “{{FORM_NAME}}”.</p>
            <p>Regards.</p>';
        }
