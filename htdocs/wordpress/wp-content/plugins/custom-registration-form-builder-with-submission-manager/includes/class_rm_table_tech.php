...
class RM_Table_Tech
{
    ...
    public function __construct()
    {
        ...
        self::$table_name_for['ROWS'] = $prefix . 'rows';
...
//      self::$table_name_for['FORM_RESPONSES'] = $prefix . 'form_responses';
        ...
        self::$table_name_for['TABS'] = $prefix . 'tabs';
        self::$table_name_for['REPORTS_NOTIFICATIONS'] = $prefix . 'reports_notifications';
        self::$table_name_for['CUSTOM_STATUS'] = $prefix . 'custom_status';
        self::$table_name_for['TASKS'] = $prefix . 'tasks';
        self::$table_name_for['RULES'] = $prefix . 'rules';
        self::$table_name_for['TASK_EXE_LOG'] = $prefix . 'task_exe_log';
    }
    ...
    public static function create_tables_per_site()
    {
        ...
        $table_name = self::get_table_name_for('ROWS');
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name)
        {
            ...
            $sql = "CREATE TABLE $table_name (
                    ...
                    columns TEXT,
                    class TEXT,
                    gutter INT(6) UNSIGNED,
                    bmargin INT(6) UNSIGNED,
                    width INT(6) UNSIGNED,
                    heading TEXT,
                    subheading TEXT,
                    field_ids TEXT,
                    row_order INT(6) UNSIGNED,
                    row_options MEDIUMTEXT,
                    PRIMARY KEY  (row_id)
                    )$charset_collate;";
        ...
        $table_name = self::get_table_name_for('TABS');
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name)
        {
            $sql = "CREATE TABLE $table_name (
        ...
        $table_name = self::get_table_name_for('REPORTS_NOTIFICATIONS');
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name)
        {
            $sql = "CREATE TABLE $table_name (
        ...
        $table_name = self::get_table_name_for('TASKS');
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name)
        {
            $sql = "CREATE TABLE $table_name (
        ...
        $table_name = self::get_table_name_for('RULES');
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name)
        {
            $sql = "CREATE TABLE $table_name (
        ...
        $table_name = self::get_table_name_for('TASK_EXE_LOG');
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name)
        {
            $sql = "CREATE TABLE $table_name (
...
//            
    ...
    public static function create_tabs_table()
    {
        if(!function_exists('dbDelta'))
        {
            if(version_compare(get_bloginfo('version'),'6.1') < 0)
    ...
    public static function create_reports_notifications_table()
    {
        if(!function_exists('dbDelta'))
        {
            if(version_compare(get_bloginfo('version'),'6.1') < 0)
    ...
    public static function create_cstatus_table(){
        if(!function_exists('dbDelta'))
        {
            if(version_compare(get_bloginfo('version'),'6.1') < 0)
                require_once( ABSPATH . 'wp-includes/wp-db.php');
