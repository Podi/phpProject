...
class RM_Utilities {
    ...
    public static function get_field_types($include_widgets= true,$form_type=1) {
        $field_types = array(
            ...
            'PGAvatar' => __('ProfileGrid Avatar','custom-registration-form-builder-with-submission-manager'),
        ...
        if ($red['redirection_type'] == 'common') {
            ...
            if (!$redirect_to){
                ...
                switch ($redirect_to) {
                    ...
                    default: $redirect_to = get_permalink(); //$redirect_to = home_url("?p=".$redirect_to);
                }
            } 
        }
    ...
    public static function get_recent_forms($service, $limit = 3) {
        $forms = $service->get_all('FORMS', $offset = 0, $limit, $column = '*', $sort_by = 'created_on', $descending = true);
        $forms_array = array();
        if(!empty($forms)) {
            foreach($forms as $form) {
                $fopts = maybe_unserialize($form->form_options);
                array_push($forms_array, array(
                    'id' => $form->form_id,
                    'name' => $form->form_name,
                    'desc' => is_null($fopts->form_description) ? '' : $fopts->form_description,
                ));
            }
        }
        return $forms_array;
    ...
    public static function get_popular_forms($service, $limit = 3) {
        $forms = $service->get_all('FORMS', $offset = 0, 999999, $column = '*', null, $descending = true);
        $forms_array = array();
...
// format date string
    ...
    public static function link_activate_user() {
        if(current_user_can('manage_options')) {
            $req = sanitize_text_field($_GET['user']);
            ...
            wp_enqueue_style(RM_PLUGIN_BASENAME, RM_BASE_URL . 'admin/css/style_rm_admin.css', array(), RM_PLUGIN_VERSION, 'all');
            wp_enqueue_style('rm_admin_common_utilit', RM_BASE_URL . 'admin/css/rm-admin-common-utility.css', array(), RM_PLUGIN_VERSION, 'all');
            if(defined('REGMAGIC_ADDON')) {
                wp_enqueue_style(RM_ADDON_PLUGIN_BASENAME, RM_ADDON_BASE_URL . 'admin/css/style_rm_admin.css', array(), RM_ADDON_PLUGIN_VERSION, 'all');
                echo '<!DOCTYPE html>
                        <html>
                        <head>
            ...
            wp_die('You need to login as admin to activate the user');
        }
    }
    ...
    public static function set_default_form() {
        if (isset($_POST['rm_def_form_id']) && current_user_can('manage_options') && check_ajax_referer( 'rm_formflow', 'rm_ajaxnonce' )) {
            $gopts = new RM_Options;
            $gopts->set_value_of('default_form_id', sanitize_text_field($_POST['rm_def_form_id']));
        }
        die;
    }
    ...
    public static function get_state() {
        if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce')) {
            ...
            if(!empty($user->ID)){
                ...
                if(empty($state)) {
                    $state= sanitize_text_field($_REQUEST['def_state']);
                }
            } else {
                $state= sanitize_text_field($_REQUEST['def_state']);
            }
            $country_arr = RM_Utilities::get_countries();
            $country_code = str_replace(array($country_arr[sanitize_text_field($_REQUEST['country'])],'[',']'),'',sanitize_text_field($_REQUEST['country']));
        ...
        if ($fields)
            foreach ($fields as $field) {
                ...
                if (isset($config['def']) && $field->field_id == $config['def'])
                    echo '<option selected value="' . esc_attr($field->field_id) . '">' . esc_html($field->field_label) . '</option>';
                else
                    echo '<option value="' . esc_attr($field->field_id) . '">' . esc_html($field->field_label) . '</option>';
            }
        if (isset($config['full']))
            echo '</select>';
        //return $options;
    }
    public static function get_condition_dropdown_option_html($config = array()) {
        $field_type = 'Textbox';
        if(isset($config['field_id']) && $config['field_id'] != ''){
            $field_model = new RM_Fields();
            $field_model->load_from_db($config['field_id']);
            $field_type = $field_model->get_field_type();
        }
        $condition_list = self::get_condition_dropdown($field_type);
        foreach($condition_list as $option => $value){
            if(isset($config['def']) && $value == $config['def']){
                echo '<option selected value="' . esc_attr($value) . '">' . esc_html($option) . '</option>';
            }
            else{
                echo '<option value="' . esc_attr($value) . '">' . esc_html($option) . '</option>';
            }
        }
    }
    public static function get_condition_values_fields($config = array()) {
        if(isset($config['field_id']) && $config['field_id'] != ''){
            $field_model = new RM_Fields();
            $field_model->load_from_db($config['field_id']);
            $field_type = $field_model->get_field_type();
            return self::get_condition_values_fields_type($field_type);
        }
        return 'text';
    }
    public static function get_condition_values_fields_type($field_type) {
        $fields = self::get_condition_values_fields_type_list();
        if(in_array($field_type, $fields['url'])){
            return 'url';
        }elseif(in_array($field_type, $fields['date'])){
            return 'date';
        }elseif(in_array($field_type, $fields['number'])){
            return 'number';
        }elseif(in_array($field_type, $fields['contact'])){
            return 'number';
        }else{
            return 'text';
        }
    ...
    public static function get_condition_values_fields_type_list() {
        if(defined('REGMAGIC_ADDON')) {
            return RM_Utilities_Addon::get_condition_values_fields_type_list();
        }
        $textFields = array('Textbox', 'Fname', 'Lname', 'Nickname', 'Custom', 'Repeatable', 'UserPassword', 'Terms', 'Username');
        $urlFields = array('Website');
        $selectFields = array('Select','Radio','Checkbox','Gender','Language','Timezone');
        $multiFields = array('Checkbox');
    ...
    public static function get_condition_dropdown($fieldType = 'Textbox') {
        $fields = self::get_condition_values_fields_type_list();
        $special_fields = array('Country','Language','Timezone');
        if(in_array($fieldType, $special_fields)){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Contains' => 'in',
        ...
        elseif(in_array($fieldType, $fields['text'])){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Contains' => 'in',
                'Empty' => '_blank',
                'Not Empty' => '_not_blank',
                'Start with word' => 'start_word',
                'End with word' => 'end_word',
                'Start with letter' => 'start_char',
                'End with letter' => 'end_char'
            );
        }
        elseif(in_array($fieldType, $fields['select'])){
            return array(
                'Empty' => '_blank',
                'Not Empty' => '_not_blank',
                'Equals' => '==',
        ...
        }elseif(in_array($fieldType, $fields['url'])){
            return array(
                'Empty' => '_blank',
                'Not Empty' => '_not_blank'
            );
        }elseif(in_array($fieldType, $fields['multi'])){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Contains' => 'in',
            );
        }elseif(in_array($fieldType, $fields['email'])){
            return array(
                'Equals' => '==',
                'Start with word' => 'start_word',
                'Start with letter' => 'start_char',
        ...
        }elseif(in_array($fieldType, $fields['date'])){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Less than or equals' => '<=',
        ...
        }elseif(in_array($fieldType, $fields['textarea'])){
            return array(
                'Contains' => 'in',
                'Empty' => '_blank',
                'Not Empty' => '_not_blank',
        ...
        }elseif(in_array($fieldType, $fields['number'])){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Less than or equals' => '<=',
        ...
        }elseif(in_array($fieldType, $fields['contact'])){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Empty' => '_blank',
                'Not Empty' => '_not_blank',
                'Start with number' => 'start_char',
                'End with number' => 'end_char'
            );
        }elseif(in_array($fieldType, $fields['dropdownObj'])){
            return array(
                'Equals' => '==',
                'Not equals' => '!=',
                'Contains' => 'in'
    ...
    public static function get_cond_action_dd($config = array()) {
        ...
        foreach($options as $key => $value){
            if(isset($config['def']) && $key == $config['def']){
                echo '<li><input type="radio" id="rm-action-'.$key.'-condition" name="action" checked value="' . esc_attr($key) . '" ></input>';
                echo '<label for="rm-action-'.$key.'-condition">' . sprintf($value) . '</label></li>';
            }
            else{
                echo '<li><input type="radio" id="rm-action-'.$key.'-condition" name="action" value="' . esc_attr($key) . '" ></input>';
                echo '<label for="rm-action-'.$key.'-condition">' . sprintf($value) . '</label></li>';
            }
        }
    ...
    public static function update_action_state_ajax() {
        if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
            $act_id = sanitize_text_field($_POST['action_id']);
    ...
    public static function get_cond_op_dd($config = array()) {
        ...
        $options = '';
        if (isset($config['full']))
            echo '<select name="' . esc_attr($config['name']) . '" id="' . (isset($config['id']) ? esc_attr($config['id']) : esc_attr($config['name'])) . '">';

        foreach ($operators as $key => $op) {
            if (isset($config['def']) && $op == $config['def'])
                echo '<option selected value="' . esc_attr($op) . '">' . esc_html($key) . '</option>';
            else
                echo '<option value="' . esc_attr($op) . '">' . esc_html($key) . '</option>';
        }
        if (isset($config['full']))
            echo '</select>';
        //return $options;
    ...
    public static function csv_excluded_widgets(){
        return array("HTMLH","Spacing","HTMLCustomized","HTML","Timer","HTMLP","Divider","Spacing","RichText","Link","YouTubeV","Iframe",'PriceV','SubCountV',"MapV","Form_Chart","FormData","Feed","ImageV","UserPassword");
    }
  ...
  public static function get_country_dial_codes(){
    ...
    public static function suspend_user(){
        if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
          ...
          $id= absint(sanitize_text_field($_REQUEST['user_id']));       
  ...
  public static function reset_password(){
      if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
        ...
        if(!$user || !$user->exists()) {
            _e('User does not exist.', 'custom-registration-form-builder-with-submission-manager');
            die;
        }
        if(is_multisite()) {
            $super_admins = get_super_admins();
            if(is_array($super_admins) && in_array($user->user_login, $super_admins, true)) {
                _e('Cannot change password of a multisite network administrator.', 'custom-registration-form-builder-with-submission-manager');
                die;
            }
        }
        $random_password = wp_generate_password($length = 12, $include_standard_special_chars = false);
        // Login to reset password
        self::quick_email(sanitize_email($_REQUEST['user_email']), __("RM LOGIN PASSWORD CHANGED",'custom-registration-form-builder-with-submission-manager'), __("Here is your updated password: ",'custom-registration-form-builder-with-submission-manager').$random_password, RM_EMAIL_GENERIC,array('do_not_save'=>true));
  ...
  public static function block_ip(){
    if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
        $ip = sanitize_text_field($_REQUEST['user_ip']); // IP of the user                   
        $gopt=new RM_Options;
  ...
  public static function block_email(){
    if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
        $email = sanitize_text_field($_REQUEST['user_email']); // IP of the user                   
        $submission_model= new RM_Submissions();
        $submission_model->block_email($email);
        //echo 'success';
        wp_send_json_success();
    }
    die;
  ...
  public static function unblock_email(){
    if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
        $email = sanitize_text_field($_REQUEST['user_email']); // IP of the user                   
        $submission_model= new RM_Submissions();
        $submission_model->unblock_email($email);
        //echo 'success';
        wp_send_json_success();
    }
    die;
  ...
  public function rm_delete_data(){
      if(check_ajax_referer('rm_ajax_secure','rm_sec_nonce') && current_user_can('manage_options')) {
          ...
          echo 'success-'.esc_html($delete_count);
      }
      die;
  }
    ...
    public static function get_form_redirection_url($form) {
        ...
        //This functionality is also part of CORE
        $redirect_url = '';
    ...
    public static function get_formatted_price($price) {
        $price = is_object($price) ? $price->total_price : $price;
        $gopts = new RM_Options;
    ...
    public static function get_formatted_sub_data($form_id, $data = array()) {
        $rows = RM_DBManager::get_rows_by_form_id($form_id);
        if(!empty($rows)) {
            $formatted_data = array();
            foreach($rows as $row) {
                $field_ids = maybe_unserialize($row->field_ids);
        ...
        return self::strip_slash_array($data);
    }
    ...
    public static function expanded_allowed_tags() {
        ...
        // legend
        $my_allowed['legend'] = array(
            'style'   => array(),
        );
        // br
        $my_allowed['br'] = array();
        // pre
        $my_allowed['pre'] = array(
            'class'   => array(),
        );
        // script
    ...
    public static function sanitize_output_data($data) {
        if(!empty($data)) {
            if(is_array($data)) {
                foreach($data as $key => $data_item) {
                    if($key == 'icon' || $key == 'mc_fields' || $key == 'input_selected_icon_codepoint') {
                        continue;
                    }
    ...
    public static function is_valid_rm_page($haystack, $needle = "rm_") {
        $length = strlen($needle);
        return substr($haystack, 0, $length) === $needle;
    }
    public static function payments_manager_included_fields(){
        return array('Fname','Lname','Email','Address','Price');
    }
    
    public static function payments_table_header_fields($fields){
        $fields_name = array();
        $fields_label = array();
        
        $fields_names = array();
        $fields_labels = array();
        $allowed_fields = self::payments_manager_included_fields();
        foreach($fields as $field){
            if(in_array($field->field_type, $allowed_fields) && isset($field->field_type)){
                $fields_name[$field->field_type] = $field->field_id;
                if($field->field_type =='Price'){
                    $fields_label[$field->field_type] = 'Product';
                }else{
                    $fields_label[$field->field_type] = $field->field_label;
                }
            }
        }
        foreach($allowed_fields as $allowed_field):
            if(isset($fields_name[$allowed_field])):
                $fields_names[] = $fields_name[$allowed_field];
            endif;
            if(isset($fields_label[$allowed_field])):
                $fields_labels[] = $fields_label[$allowed_field];
            endif;
        endforeach;
        
        $data = new stdClass();
        $data->field_names = $fields_names;
        $data->field_labels = $fields_labels;
