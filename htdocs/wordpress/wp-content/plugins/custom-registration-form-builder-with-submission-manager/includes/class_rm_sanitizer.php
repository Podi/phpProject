...
class RM_Sanitizer
{
	...
	public function sanitize_request($req)
	{
        ...
        if(isset($_GET['page']) && is_array($request) && is_string($_GET['page']))
            $request['page'] = sanitize_text_field($_GET['page']);
    ...
    public function get_sanitized_rm_options_add_ctabs_page($key, $value)
    {
         switch($key)
        {
            case 'ctab_desc':
    ...
    public function get_sanitized_rm_options_manage_invoice_page($key, $value){
        switch($key)
        {
