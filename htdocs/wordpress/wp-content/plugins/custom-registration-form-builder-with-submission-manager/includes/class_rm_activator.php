...
class RM_Activator
{
    ...
    public static function activate($network_wide)
    {
        ...
        if (method_exists('RM_Chronos_Service', 'insert_cron_on_activate_plugin')){
            ...
            $cron_service->insert_cron_on_activate_plugin();
        }
    }
    ...
    {   
        ...
        self::migrate_rm_5_5_to_rm_5_6($network_wide, $existing_rm_db_version);
        self::migrate_rm_5_6_to_rm_5_7($network_wide, $existing_rm_db_version);
        self::migrate_rm_5_7_to_rm_5_8($network_wide, $existing_rm_db_version);
        self::migrate_rm_5_8_to_rm_5_9($network_wide, $existing_rm_db_version);
        //self::migrate_rm_5_9_to_rm_6_0($network_wide, $existing_rm_db_version);
    ...
    private static function migrate_rm_5_5_to_rm_5_6($network_wide, $existing_rm_db_version){
        global $wpdb;
        if (floatval($existing_rm_db_version) < 5.5)
        {
            if (is_multisite() && $network_wide)
            {
                ...
                {
                    switch_to_blog($blog_id);
                    RM_Table_Tech::create_rows_table();
                    restore_current_blog();
                }
            }
            else
    ...
    private static function migrate_rm_5_6_to_rm_5_7($network_wide, $existing_rm_db_version){
        global $wpdb;
        if (floatval($existing_rm_db_version) < 5.7)
        {
            if (is_multisite() && $network_wide)
            {
                $current_blog = $wpdb->blogid;
    ...
    private static function migrate_rm_5_7_to_rm_5_8($network_wide, $existing_rm_db_version){
        global $wpdb;
        if (floatval($existing_rm_db_version) < 5.8)
        {
            if (is_multisite() && $network_wide)
            {
                $current_blog = $wpdb->blogid;
                ...
                {
                    switch_to_blog($blog_id);
                    RM_Table_Tech::create_reports_notifications_table();
                    restore_current_blog();
                }
            }
            else
    ...
    private static function migrate_rm_5_8_to_rm_5_9($network_wide, $existing_rm_db_version){
        global $wpdb;
        if (floatval($existing_rm_db_version) < 5.9)
        {
            if (is_multisite() && $network_wide)
            {
                $current_blog = $wpdb->blogid;
                ...
                {
                    switch_to_blog($blog_id);
                    RM_Table_Tech::create_cstatus_table();
                    restore_current_blog();
                }
            }
            else
    ...
    private static function migrate_rm_5_9_to_rm_6_0($network_wide, $existing_rm_db_version){
        global $wpdb;
        if (floatval($existing_rm_db_version) < 6.0)
        {
            ...
            if (is_multisite() && $network_wide)
            {
                ...
                foreach ($blog_ids as $blog_id)
                {
                    switch_to_blog($blog_id);
                    foreach($forms as $form) {
                        $service->add_default_cstatuses($form);
                        $service->add_default_crons($form);
                    }
                    restore_current_blog();
                }
            }
...
//ob_start();
    ...
    public static function first_install_proc()
    {
        ...
        if(!$existing_rm_db_version)
        {
            ...
            foreach($sample_products as $prod => $price) {
                $wpdb->insert(RM_Table_Tech::get_table_name_for('PAYPAL_FIELDS'), array(
                    'type' => 'fixed',
                    'name' => $prod,
                    'value' => $price,
                    'extra_options' => 'a:4:{s:12:"show_on_form";s:3:"yes";s:14:"allow_quantity";s:2:"no";s:12:"min_quantity";s:1:"0";s:12:"max_quantity";s:1:"0";}',
                ), array('%s','%s','%s','%s'));
            }
