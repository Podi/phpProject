        ...
        if(!empty($this->field_model->field_options->conditions['rules']) && is_array($this->field_model->field_options->conditions['rules'])){
            ...
            foreach($conditions as $condition)
            {
                ...
                if($cf_field->load_from_db($cf_id)){
                    ...
                    else if($cType=="Language" && defined('REGMAGIC_ADDON'))
                    {
                        $pfbc_field= new Element_Timezone("tz","tz");
                    ...
                    }else if($cType == 'Username' || $cType == 'UserPassword'){
                        $field_name= strtolower($cType);
                        if($cType == 'UserPassword') {$field_name = 'pwd';}
                        if(is_array($condition['values'])){
                            $values= implode(',', $condition['values']);
                        }
                        else{
            ...
            if(count($cond_option)>0){
                ...
                $this->field_options['data-cond-action']= isset($this->field_model->field_options->conditions['action']) ? $this->field_model->field_options->conditions['action'] : 'show';
    ...
    public function get_prepared_data($request)
    {
        ...
        if(isset($request[$this->field_name])) {
            if(is_array($request[$this->field_name])) {
                foreach($request[$this->field_name] as $key => $val) {
                    if(is_array($val)) {
                        $request[$this->field_name][$key] = array_map('sanitize_textarea_field', $val);
                    } else {
                        $request[$this->field_name][$key] = sanitize_textarea_field($val);
                    }
                }
            } else {
                $request[$this->field_name] = sanitize_textarea_field($request[$this->field_name]);
            }
            $data->value = $request[$this->field_name];
        } else {
            $data->value = null;
        }
