...
<div class="rmagic" id="rm_front_sub_tabs" style="display:none;"> 
    ...
    <div class="operationsbar">
        ...
        <div class="nav">
                ...
                <div class="rm_back_button rmtabs_head" title="<?php echo RM_UI_Strings::get('LABEL_BACK'); ?>" data-rmt-tabcontent="__rmt_noop" onclick="window.location.href = location.protocol + '//' + location.host + location.pathname"><i class="material-icons">keyboard_arrow_left</i> <?php echo RM_UI_Strings::get('LABEL_BACK'); ?></div>
    ...
    <?php
        ...
        if ($data->payment) {
            echo do_action('rm_submission_before_submission_view',$data->submission->get_form_id(), $data->submission->get_submission_id(), $data->payment->id);
        }
                ...
                <div class="rm-submission-field-row rm-submission-status-row">
                    <div class="rm-submission-label rm-custom_status-wrap">
                        ...
                        foreach($custom_statuses as $custom_status){
                            echo '<div class="rm-custom-status" style="background-color: #'.$form_options->custom_status[$custom_status->status_index]['color'].'">'.$form_options->custom_status[$custom_status->status_index]['label'].'<span style="border-color: #'.$form_options->custom_status[$custom_status->status_index]['color'].'"></span></div>';
                        }
                        ?>
                    </div>
                </div>
...
if ($data->form_is_unique_token) {
    ?>
            <div class="rm-submission-field-row">
                ...
                <div class="rm-submission-value"><?php echo esc_html($data->submission->get_unique_token()); ?></div>
            </div>
    <?php
}
?>
        ...
        <div class="rm-submission-field-row">
            ...
            <div class="rm-submission-value"><?php echo esc_html($data->submission->get_submission_id()); ?></div>
        </div>
                ...
                foreach ($sub_data as $sub) {
                    ...
                    ?>
                                    ...
                                    <?php echo wp_get_attachment_link($sub, 'thumbnail', false, true, false); ?>
                                        ...
                                        <div class="rm-submission-attachment-field"><a href="<?php echo esc_url($att_url); ?>"><?php echo RM_UI_Strings::get('LABEL_DOWNLOAD'); ?></a></div>
                                    </div>
    ...
    if ($data->payment->log):
        ?>
            ...
            if (isset($data->payment->log['first_name']))
                echo esc_html($data->payment->log['first_name']);
            if (isset($data->payment->log['last_name']))
                echo ' ' . esc_html($data->payment->log['last_name']);
            ?></div>
                </div>
                <div class="rm-submission-field-row">
                    <div class="rm-submission-label"><?php echo RM_UI_Strings::get('LABEL_PAYER_EMAIL'); ?></div>
                    <div class="rm-submission-value"><?php if (isset($data->payment->log['payer_email'])) echo sanitize_email($data->payment->log['payer_email']); ?></div>
                </div>
                        <?php
                    endif;
                    ?>
            ...
            <div class="rm-submission-field-row">
                ...
                <div class="rm-submission-value"><?php if (isset($data->payment->status)) echo esc_html($data->payment->status); ?></div>
            </div>
            <div class="rm-submission-field-row">
...
if ($data->notes && (is_object($data->notes) || is_array($data->notes))) {
    ...
    foreach ($data->notes as $note) {
        ?>
            ...
            if ($note->editor)
                echo " (" . RM_UI_Strings::get('LABEL_EDITED_BY') . " <b>" . esc_html($note->editor) . "</b> <em>" . RM_Utilities::localize_time($note->last_edit_date) . "</em>";
            ?>
                </div>
            </div>
                    <?php
